<template>
  ...
  <div id="box">
    ...
    <el-button class="title" @click.prevent="partyAll" type="danger">{{partySummary}}</el-button>
    <div class="svg_tag">
...
setup(){
  ...
  onMounted(async ()=>{
    ...
    if(res.status===200 && res.data.code ===0){
      // console.log(res.data.data.filter(item=> item.bool=== true )[0].name)
      party.data =res.data.data.filter(item=> item.bool=== false )
      partySummary.value =res.data.data.filter(item=> item.bool=== true )[0].name || ''
    }
  })
  return{
    partySummary,
